//
//  VisitTeamLineupViewController.swift
//  scorebook
//
...
class VisitTeamLineupViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    
    var teams = [TeamInfo]()
    var visitedTeam = TeamInfo()
    ...
    //원정 팀명
    @IBOutlet weak var lblVisitedTeamName: UILabel!
    ...
    @IBAction func clickSetLineupComplete(_ sender: Any) {
        ...
        self.visitedTeam.mainLineupInfo = self.selectedLineups
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath){
        ...
        case self.selectedLineupTableView:
            ...
            let index = self.visitedTeam.lineupInfo.index{$0.name == cell.lblName.text}
            if index != nil {
    ...
    func getData() {
        ...
        if self.teams.first(where: {$0.kind == "V"}) != nil {
            self.visitedTeam = self.teams.first(where: {$0.kind == "V"})!
//            self.selectedLineups = self.visitedTeam.mainLineupInfo
            
            //            if self.visitedTeam.lineupInfo.count < 9 {
            //                let alert = UIAlertController(title: "확인", message: "라인업 설정을 먼저 하세요.", preferredStyle: UIAlertControllerStyle.alert)
            //                alert.addAction(UIAlertAction(title: "OK", style: UIAlertActionStyle.default, handler: nil))
            //                self.present(alert, animated: true, completion: nil)
            //                return
            //            }
            
            self.lblVisitedTeamName.text = self.visitedTeam.name

            // 버그 있을때 초기화
//            self.visitedTeam.mainLineupInfo = [Lineup]()
//            let userDefaults = UserDefaults.standard
//            let encodedData: Data = NSKeyedArchiver.archivedData(withRootObject: self.teams)
//            userDefaults.set(encodedData, forKey: "Teams")
//            userDefaults.synchronize()
            
            if self.visitedTeam.lineupInfo.count > 0 {
                for i in 0...self.visitedTeam.lineupInfo.count - 1 {
                    self.lineups.append(self.visitedTeam.lineupInfo[i])
                }
            }
            
            //기존 등록된 라인업이 있다면 셋팅
            if self.visitedTeam.mainLineupInfo.count != 0  {
                for i in 0...self.visitedTeam.mainLineupInfo.count - 1 {
                    self.selectedLineups.append(self.visitedTeam.mainLineupInfo[i])
                    let name = self.visitedTeam.mainLineupInfo[i].name
                    let number = self.visitedTeam.mainLineupInfo[i].memberNumber
                    let positionName = self.visitedTeam.mainLineupInfo[i].positionName
                    if self.visitedTeam.mainLineupInfo[i].isSetPosition == true {
                        let btnSender = self.getPositionButtonByPositionName(positionName: positionName)
                        self.loadPosition(sender: btnSender)
                    }
                    
                    //포지션이 투수일때는 투수 정보란에 입력
                    if self.visitedTeam.mainLineupInfo[i].positionName == "SP" {
